...
public class Demo9_EventTimeAndWatermark2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ds.process(new ProcessFunction<Row, String>() {
            @Override
            public void processElement(Row row, ProcessFunction<Row, String>.Context ctx, Collector<String> out) {
                out.collect(row + " => " + ctx.timerService().currentWatermark());
            }
        })/*.print()*/;


        // 可以在 流转表 时，显式声明watermark策略
        tenv.createTemporaryView("t_events2", s2, Schema.newBuilder()
                ...
                .watermark("rt", "source_watermark()") // 声明 watermark 直接引用底层流的watermark
                .build());
