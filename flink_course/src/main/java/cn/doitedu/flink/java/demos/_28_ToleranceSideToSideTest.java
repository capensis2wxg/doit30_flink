...
public class _28_ToleranceSideToSideTest {
    public static void main(String[] args) throws Exception {
        ...
        KafkaSource<String> sourceOperator = KafkaSource.<String>builder()
                .setBootstrapServers("master102:9092")
                .setTopics("eos")
                .setGroupId("eos01")
                .setValueOnlyDeserializer(new SimpleStringSchema())
                .setProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "false")  // 允许kafkaConsumer自动提交消费位移到 __consumer_offsets
                // kafkaSource进行状态checkpoint时，默认会向__consumer_offsets提交一下状态中记录的偏移量
                // 但是,flink的容错并不优选依赖__consumer_offsets中的记录，所以可以关闭该默认机制
                .setProperty("commit.offsets.on.checkpoint", "false") // 默认是true
                // kafkaSource启动时，获取起始位移的策略设置，如果是committedOffsets ，则是从之前所记录的偏移量开始
        ...
        SinkFunction<String> exactlyOnceJdbcSink = JdbcSink.exactlyOnceSink(
                ...
                new SerializableSupplier<XADataSource>() {
                    ...
                    public XADataSource get() {
                        ...
                        xaDataSource.setUrl("jdbc:mysql://master102:3306/abc");
                        xaDataSource.setUser("root");
                        xaDataSource.setPassword("wxg@124328l");
                        return xaDataSource;
                    }
                }
        );
        ...
        SingleOutputStreamOperator<String> stream2 = stream1.keyBy(s -> "group1")
                .map(new RichMapFunction<String, String>() {
                    ...
                    public void open(Configuration parameters) {
                        valueState = getRuntimeContext().getState(new ValueStateDescriptor<>("preStr", String.class));
                    }
