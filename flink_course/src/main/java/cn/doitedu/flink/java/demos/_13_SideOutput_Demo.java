...
public class _13_SideOutput_Demo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<EventLog> processed = streamSource.process(new ProcessFunction<EventLog, EventLog>() {
             ...
             */
            @Override
            public void processElement(EventLog eventLog, ProcessFunction<EventLog, EventLog>.Context ctx, Collector<EventLog> out) {
                String eventId = eventLog.getEventId();
                if ("appLaunch".equals(eventId)) {
                    ctx.output(new OutputTag<>("launch", TypeInformation.of(EventLog.class)), eventLog);
                } else if ("putBack".equals(eventId)) {
                    ctx.output(new OutputTag<>("back", TypeInformation.of(String.class)), JSON.toJSONString(eventLog));
                }
