...
class MyEventTimeTrigger extends Trigger<Tuple2<EventBean2,Integer>, TimeWindow>{
    ...
    public TriggerResult onElement(
        ...
        if (window.maxTimestamp() <= ctx.getCurrentWatermark()) {
            ...
            // 注册定时器，定时器的触发时间为：窗口的结束点时间,time – The watermark at which to invoke onEventTime(long, Window, Trigger.TriggerContext)
            ctx.registerEventTimeTimer(window.maxTimestamp());
            // 判断，当前数据的用户行为事件id是否等于e0x，如是则触发
     ...
     * 下面的方法，定时器一直在调用<br>
     * <strong>time – The watermark at which to invoke onEventTime(long, Window, Trigger.TriggerContext)</strong>
     */
    @Override
