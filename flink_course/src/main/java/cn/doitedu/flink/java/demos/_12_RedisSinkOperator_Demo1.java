...
// * @QQ: 657270652
// * @Date: 2022/4/26
// * @Desc: 将数据流写入redis，利用RedisSink算子
// **/
...
//        FlinkJedisPoolConfig config = new FlinkJedisPoolConfig.Builder().setHost("master102").build();
//
...
//
//    /**
//     *  如果选择的是没有内部key的redis数据结构，则此方法返回的就是外部key <p>
//     *  如果选择的是有内部key的redis数据结构(hset)，则此方法返回的是hset内部key，而把上面Description中传入的值作为外部key
//     * @param data 流入数据
//     * @return redis 数据类型
//     */
//
...
//        // 可以根据具体数据，选择额外key（就是hash这种结构，它有额外key(外部key)
//        @Override
...
//
//    /**
//    *  如果选择的是没有内部key的redis数据结构，则此方法返回的就是大key
...
//        @Override
//        public String getKeyFromData(EventLog data) {
//            return data.getGuid()+"-"+data.getSessionId()+"-"+data.getTimeStamp();  // 这里就是hset中的field(内部key)
//        }
//
//        @Override
//        public String getValueFromData(EventLog data) {
