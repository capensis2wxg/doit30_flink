...
public class _20_Window_Api_Demo1 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /*
          滚动聚合api使用示例
          需求一： 每隔10s，统计最近30s的数据中每个用户的行为事件条数
          使用aggregate算子来实现
         */
        SingleOutputStreamOperator<Integer> resultStream = watermarkedBeanStream
                ...
                // reduce：滚动聚合算子的限制, 聚合结果的数据类型与数据源中的数据类型必须一致
                ...
                .aggregate(new AggregateFunction<EventBean2, Integer, Integer>() {
                    ...
                    /**
                     *  批计算模式下，可能需要将多个上游的局部聚合累加器，放在下游进行全局聚合, 因此需要对两个累加器进行合并
                     *  这里就是合并的逻辑
                     *  流计算模式下，不用实现！
        ...
        /*
         * 需求二：每隔10s，统计最近 30s 的数据中每个用户的平均每次行为时长
         * 要求用aggregate算子来做聚合
         * 滚动聚合api使用示例
         */
        SingleOutputStreamOperator<Double> resultStream2 = watermarkedBeanStream
                ...
                .aggregate(new AggregateFunction<EventBean2, Tuple2<Integer, Integer>, Double>() {
                    ...
                    /**
                     * 在批计算模式中，shuffle的上游可以做局部聚合，然后会把局部聚合结果交给下游去做全局聚合。因此，就需要提供两个局部聚合结果进行合并的逻辑
        ...
        /*
         * TODO 补充练习 1
         * 需求一：每隔10s，统计最近 30s 的数据中每个用户的行为事件条数
         * 滚动聚合api使用示例
         * 使用sum算子来实现
         */
        ...
        /*
         * 全窗口计算api使用示例
         * 需求三：每隔10s，统计最近 30s 的数据中，每个用户的行为事件中行为时长最长的前2条记录
        ...
        SingleOutputStreamOperator<EventBean2> resultStream3 = watermarkedBeanStream.keyBy(EventBean2::getGuid)
                ...
                .apply(new WindowFunction<EventBean2, EventBean2, Long, TimeWindow>() {
                    /**
                     * @param key 本次传给的窗口是属于哪个key的
                     * @param window 本次传给的窗口的各种元信息（比如本窗口的起始时间，结束时间）
                     * @param input 本次传给的窗口中所有数据的迭代器
                     * @param out 结果数据输出器
                     */
                    @Override
                    public void apply(Long key, TimeWindow window, Iterable<EventBean2> input, Collector<EventBean2> out) {
                        ...
                        // 排序
                        tmpList.sort(new Comparator<EventBean2>() {
                            @Override
